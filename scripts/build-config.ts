...
function buildConfigTs(config: Config, configHash: string, bootstrapName: string, widgetName: string): string {
...
export async function buildConfig(opts: BuildOptions): Promise<void> {
  ...
  const configTs = buildConfigTs(config, configHash, bootstrapName, widgetName);
